...
namespace fredyns\attachment\behaviors;
...
class FileBehavior extends Behavior
{
    ...
    public function getInitialPreview()
    {
        ...
        foreach (FileHelper::findFiles($userTempDir) as $file) {
            if (substr(FileHelper::getMimeType($file), 0, 5) === 'image') {
                $initialPreview[] = Html::img(['/attachment/file/download-temp', 'filename' => basename($file)], ['class' => 'file-preview-image']);
            } else {
    ...
    public function getInitialPreviewConfig()
    {
        ...
        foreach (FileHelper::findFiles($userTempDir) as $file) {
            ...
            $initialPreviewConfig[] = [
                'caption' => $filename,
                'url' => Url::to(['/attachment/file/delete-temp',
                    'filename' => $filename
                ]),
            ];
        }
        ...
        foreach ($this->getFiles() as $index => $file) {
            $initialPreviewConfig[] = [
                ...
                'url' => Url::toRoute(['/attachment/file/delete',
                    'id' => $file->id
                ]),
            ];
        }
